...
function renderCartModal(cart) {
    ...
    modalItems.innerHTML = items.map(item => {
        const imageHtml = item.image_url ?
            `<img src="/public/images/products/${item.image_url}" alt="${item.name}" class="w-16 h-16 rounded-lg object-cover bg-slate-700/50">` :
    ...
    modalTotal.textContent = '$' + (total / 100).toFixed(2);
