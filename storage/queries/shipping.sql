...
SELECT
    ...
    SUM(CASE WHEN category = 'large' THEN COALESCE(weight_oz,0) * quantity ELSE 0 END) as large_weight_oz,
    SUM(CASE WHEN category = 'xlarge' THEN COALESCE(weight_oz,0) * quantity ELSE 0 END) as xlarge_weight_oz,
    SUM(CASE WHEN category = 'small' AND (weight_oz IS NULL OR weight_oz <= 0 OR length_in IS NULL OR length_in <= 0 OR width_in IS NULL OR width_in <= 0 OR height_in IS NULL OR height_in <= 0) THEN quantity ELSE 0 END) as small_missing_shipping,
    SUM(CASE WHEN category = 'medium' AND (weight_oz IS NULL OR weight_oz <= 0 OR length_in IS NULL OR length_in <= 0 OR width_in IS NULL OR width_in <= 0 OR height_in IS NULL OR height_in <= 0) THEN quantity ELSE 0 END) as medium_missing_shipping,
    SUM(CASE WHEN category = 'large' AND (weight_oz IS NULL OR weight_oz <= 0 OR length_in IS NULL OR length_in <= 0 OR width_in IS NULL OR width_in <= 0 OR height_in IS NULL OR height_in <= 0) THEN quantity ELSE 0 END) as large_missing_shipping,
    SUM(CASE WHEN category = 'xlarge' AND (weight_oz IS NULL OR weight_oz <= 0 OR length_in IS NULL OR length_in <= 0 OR width_in IS NULL OR width_in <= 0 OR height_in IS NULL OR height_in <= 0) THEN quantity ELSE 0 END) as xlarge_missing_shipping,
